import numpy as np
import streamlit as st
import joblib
import warnings
warnings.filterwarnings("ignore")

model = joblib.load("C:\\Users\\Admin\\Desktop\\python\\Exam_score_predictor_using_ML\\Notebooks\\best_model_pkl")

st.title("Student Exam Score Predictor")

study_hours = st.slider("Study hours per Day", 0.0, 12.0, 2.0)
attendence = st.slider("Attendence percentage", 0.0, 100.0, 80.0)
mental_health = st.slider("Mental Health Rating", 1, 10, 5)
sleep_hours = st.slider("Sleep Hours per Night", 0.0, 12.0, 7.0)
part_time_job = st.selectbox("Part-Time job",["No", "Yes"])

ptj_encoded = 1 if part_time_job == "Yes" else 0

if st.button("Predict Exam Score"):
    input_data = np.array([study_hours, attendence, mental_health, sleep_hours, ptj_encoded]).reshape(1, -1)
    prediction = model.predict(input_data)[0]


    prediction = max(0, min(100, prediction))

    st.success(f"Predicted Exam score: {prediction:.2f}")